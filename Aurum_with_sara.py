...
if "user" in st.session_state:
    ...
    if st.session_state["data_source_choice"] == "📤 Upload file":
        ...
        try:
            ...
            df_user = pd.DataFrame(all_records)
            df_user = df_user[df_user["Author"] == st.session_state["user"]]
            if df_user.empty:
                st.warning("You don't have any submitted cases.")
            else:
                df = df_user.copy()
                ...
                st.success("✅ Data from My Cases loaded successfully.")

        except Exception as e:
            st.error(f"❌ Failed to load your submitted cases: {e}")
