    ...
    try:
        ...
        st.error(f"❌ Error reading file: {e}")
            ...
            if show_network:
                ...
                st.markdown("This network shows cases as nodes connected by co-occurring species.")

                # Criar grafo baseado em coocorrência de espécies por caso
                G = nx.Graph()
                for case_id, group in df_selected.groupby("Case #"):
                    species_in_case = group['Species'].unique()
                    for sp1, sp2 in combinations(species_in_case, 2):
                        G.add_edge(sp1, sp2, case=case_id)
